...
//! relying solely on application-level ping/pong timeouts. It exposes a
//! poll-friendly [`WsSender`]/[`WsReceiver`] pair drivable from a synchronous
//! per-frame loop: outbound frames are queued to a background tokio task, and
//! inbound [`WsEvent`]s are drained non-blocking via [`WsReceiver::try_recv`].
//!
//! This replaces the previous `ewebsock` dependency. We deliberately do not
//! support a wasm/browser backend — nostrdb-net is native only, and owning the
...
/// Inbound half of a relay websocket connection, drained from the poll loop.
...
/// schedule a poll-loop drain. Requires an active tokio runtime.
